...
function RSVP({ match }) {
    ...
    useEffect(() => {
        const fetchRSVPs = async () => {
            try {
                const response = await axios.get(`https://rsvp-app-backend.onrender.com/event/${match.params.id}/rsvps`, {
        ...
        const fetchEvent = async () => {
            try {
                const response = await axios.get(`https://rsvp-app-backend.onrender.com/event/${match.params.id}`, {
                    headers: { 'ngrok-skip-browser-warning': 'true' }
                });
    ...
    const handleRSVP = async () => {
        try {
            await axios.post(`https://rsvp-app-backend.onrender.com/rsvp/${match.params.id}`, { attendeeName: name });
